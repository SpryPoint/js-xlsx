...
dist: dist-deps $(TARGET) bower.json
	cp $(TARGET) dist/
	cp LICENSE dist/
	uglifyjs $(TARGET) -o dist/$(LIB).min.js --source-map dist/$(LIB).min.map --source-map-url $(LIB).min.map --preamble "$$(head -n 1 bits/00_header.js)"
	uglifyjs $(REQS) $(TARGET) -o dist/$(LIB).core.min.js --source-map dist/$(LIB).core.min.map --source-map-url $(LIB).min.map --preamble "$$(head -n 1 bits/00_header.js)"
	uglifyjs $(REQS) $(ADDONS) $(TARGET) -o dist/$(LIB).full.min.js --source-map dist/$(LIB).full.min.map --source-map-url $(LIB).min.map --preamble "$$(head -n 1 bits/00_header.js)"
